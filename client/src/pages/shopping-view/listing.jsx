...
const ShoppingListing = () => {
  ...
  // function handleFilter(getSectionId, getCurrentProductId) {
  //   let cpyFilter = { ...filter };

  //   const indexOFCurrentOption =
  //     Object.keys(getSectionId).findIndex(getCurrentProductId);

  //   if (indexOFCurrentOption === -1) {
  //     cpyFilter = {
  //       ...cpyFilter,
  //       [getSectionId]: [getCurrentProductId],
  //     };
  //   } else {
  //     let indexOFCurrentOption =
  //       cpyFilter[getSectionId].indexOf(getCurrentProductId);

  //     if (indexOFCurrentOption === -1)
  //       cpyFilter[getSectionId].push(getCurrentProductId);
  ...
  function handleFilter(getSectionId, getCurrentProductId) {
    ...
    let currentCopyFilterIndex = Object.keys(cpyFilter).findIndex(getSectionId);

    if (currentCopyFilterIndex === -1) {
      cpyFilter = {
        ...cpyFilter,
        [getSectionId]: [getCurrentProductId],
      };
    } else {
      let currentCopyFilterIndex =
        cpyFilter[getSectionId].indexOf(getCurrentProductId);
      if (currentCopyFilterIndex === -1) {
        cpyFilter[getSectionId].push(getCurrentProductId);
      }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-[200px_1fr] gap-6 p-4 md:p-6">
      <ProductFilter filters={filter} />
      <div className="bg-background w-full rounded-lg shadow-sm">
        <div className="p-4 border-b flex items-center justify-between">
          <h2 className="text-lg font-extrabold">All Products</h2>
          <div className="flex items-center gap-3">
            <span className="text-muted-forefround">
              {productList?.length} Products
            </span>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent>
                <DropdownMenuGroup>
                  {sortOptions.map((sortItem) => (
                    <DropdownMenuRadioItem key={sortItem.value}>
                      {sortItem.label}
                    </DropdownMenuRadioItem>
                  ))}
                </DropdownMenuGroup>
